...
- **Desktop shell:** Rust + Tauri 2 / Wry + Microsoft WebView2 on Windows.
- **Browser isolation:** WebView-specific `http://` or `socks5://` proxy; no system TUN is required for the default mode.
...
- **Alternative universal engine:** sing-box is technically excellent, but its GPLv3 distribution implications require an explicit product/licensing decision before it becomes the default bundled engine.
...
- **Authorization:** SecureAcces remains server-side. WebGate is a secure network/application client, not a replacement authorization database.

## Why app-local transport instead of a normal VPN

A traditional VPN changes routing at the OS level. WebGate instead binds a proxy directly to its embedded browser:
...
The repository starts documentation-first. Code should only be committed after the transport, trust-boundary and configuration contracts are explicit enough to test.

## Non-negotiable security properties

- fail closed on transport failure;
- no automatic direct-connect fallback from the protected WebView;
...
- protected WebView has an explicit origin allowlist;
- external navigation goes to the system browser, never through the protected transport;
- debug/devtools are disabled in production;
...
Primary sources and the comparative decision matrix are tracked in [`docs/research/TOOLING_AUDIT.md`](docs/research/TOOLING_AUDIT.md).

## Status

**Phase 0 — architecture/tooling research started.**
