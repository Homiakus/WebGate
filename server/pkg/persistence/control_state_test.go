...
func TestControlStatePersistsAuditConfigAndBackupRestore(t *testing.T) {
	...
	registryStore, err := OpenSQLiteRegistryStore(dbPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	event := domain.AuditEvent{ID: "audit-0001", Action: domain.AuditActionServiceUpdated, ActorID: "admin", TargetID: "config", Details: "durable change", Timestamp: time.Now().UTC()}
	...
	if err := RestoreSQLiteBackup(backupPath, restoredPath); err != nil {
		t.Fatal(err)
	}
	restoredRegistry, err := OpenSQLiteRegistryStore(restoredPath)
	if err != nil {
		t.Fatal(err)
	}
	defer restoredRegistry.Close()
	restoredControl, err := OpenSQLiteControlStore(restoredRegistry)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAuditTableIsAppendOnly(t *testing.T) {
	registryStore, err := OpenSQLiteRegistryStore(filepath.Join(t.TempDir(), "state.db"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := control.db.Exec(`UPDATE audit_events SET checksum = 'tampered' WHERE event_id = ?`, event.ID); err == nil {
		t.Fatal("audit UPDATE unexpectedly succeeded")
	}
	if _, err := control.db.Exec(`DELETE FROM audit_events WHERE event_id = ?`, event.ID); err == nil {
